...
import { chemistryFor, chemistryOptions } from "@/lib/chemistry";

export function BirthdayChemistry({
  minYear,
  maxYear,
  initialYear,
  onYearChange,
  ...
  const options = useMemo(
    () => chemistryOptions(yearMin, yearMax),
    [yearMin, yearMax],
  );
  ...
  return (
    <div className="w-full text-center">
      ...
      <p className="mt-2 font-sans text-base text-zinc-500">{chemistryFor(safeYear)}</p>
    </div>
  );
}
